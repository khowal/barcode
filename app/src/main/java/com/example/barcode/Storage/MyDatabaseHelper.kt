...
class MyDatabaseHelper(context: Context) :
    ...
    companion object {
        ...
        const val TABLE_NAME = "BarcodeData"
        const val COLUMN_ID = "id"
        const val COLUMN_USER_NAME = "user_name"
        const val COLUMN_USER_IMAGE_PATH = "user_image_path"
        const val COLUMN_USER_BARCODE = "user_barcode"
        const val COLUMN_STATUS = "status" // 1 - > Done , 0 -> remaining
    }
    ...
    fun fetchRemainingUploads(isLimit: Boolean): List<Map<String, Any>> {
        val db = readableDatabase
        val cursor: Cursor
        if (isLimit == true) {
            cursor = db.rawQuery(
                "SELECT * FROM $TABLE_NAME WHERE $COLUMN_STATUS = ? LIMIT 1", arrayOf("0")
            )
        } else {
            cursor = db.rawQuery("SELECT * FROM $TABLE_NAME WHERE $COLUMN_STATUS = ?", arrayOf("0"))
        }
